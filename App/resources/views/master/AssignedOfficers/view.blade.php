						...
						<div class="card-header text-center">
							<div class="form-row  d-flex justify-content-center">
								...
								<div class="col-md-4 my-2 text-right text-md-right">
									...
									@if($crud['add']==1)
										<a href="{{ url('/') }}/master/AssignedOfficers/create" class="btn  btn-outline-primary btn-air-success btn-sm" type="button" >Create</a>
									@endif
									@if($crud['add']==1)
										<a href="{{ url('/') }}/master/AssignedOfficers/Import" class="btn  btn-outline-success btn-air-success btn-sm" type="button" >Import</a> <!-- full-right -->
									@endif
								</div>
							</div>
						</div>
    ...
    $(document).ready(function(){
        ...
        const LoadTable=async()=>{
			...
			$('#tblAssignedOfficers').dataTable( {
				"bProcessing": true,
				"bServerSide": true,
                "ajax": {"url": RootUrl+"master/AssignedOfficers/data?_token="+$('meta[name=_token]').attr('content'),"headers":{ 'X-CSRF-Token' : $('meta[name=_token]').attr('content') } ,"type": "POST"},
				deferRender: true,
				responsive: true,
				dom: 'Bfrtip',
				"iDisplayLength": 10,
		...
		$(document).on('click','.btnEdit',function(){
			window.location.replace("{{url('/')}}/master/AssignedOfficers/edit/"+ $(this).attr('data-id'));
		});
		...
		$(document).on('click','.btnDelete',function(){
            ...
            function(){swal.close();
            	$.ajax({
            		type:"post",
                    url:"{{url('/')}}/master/AssignedOfficers/delete/"+ID,
