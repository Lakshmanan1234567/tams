...
<div class="container-fluid">
	<div class="row d-flex justify-content-center">
		<div class="col-sm-6">
			<div class="card">
				<div class="card-header text-center"><h5 class="mt-10">Payment Initiated</h5></div>
				<div class="card-body " >
                    ...
                    <div class="row">
                        <div class="col-sm-12 text-right">
                            @if($crud['view']==true)
                            <a href="{{url('/')}}/master/paymentinitiated/" class="btn btn-sm btn-outline-dark" id="btnCancel">Back</a>
                            @endif
...
<script>
    ...
    return status;
        ...
        const getHT=async()=>{
			...
			$.ajax({
				type:"post",
				// url:"{{url('/')}}/master/paymentinitiated/gethp/"+editHTID,
                url:"{{url('/')}}/master/paymentinitiated/gethp/"+editHTID,
				headers: { 'X-CSRF-Token' : $('meta[name=_token]').attr('content') },
                dataType:"json",
                error:function(e, x, settings, exception){ajax_errors(e, x, settings, exception);},
        ...
        const Gethousephase=async()=>{
			...
			$.ajax({
				type:"post",
                url:"{{url('/')}}/master/paymentinitiated/Gethousephase/"+editid,
				headers: { 'X-CSRF-Token' : $('meta[name=_token]').attr('content') },
                dataType:"json",
                error:function(e, x, settings, exception){ajax_errors(e, x, settings, exception);},
                complete: function(e, x, settings, exception){btnReset($('#btnSave'));ajaxindicatorstop();},
        ...
        $('#btnSave').click(function(){
            ...
            if(status){
                swal({
                    ...
                    swal.close();
                    btnLoading($('#btnSave'));
                    let postUrl="{{url('/')}}/master/paymentinitiated/create";
                    ...
                    @if($isEdit==true)
                        ...
                        postUrl="{{url('/')}}/master/paymentinitiated/create";
                    @endif
                    $.ajax({
                        type:"post",
                        url:postUrl,
